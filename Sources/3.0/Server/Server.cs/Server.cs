    ...
    public class Server
    {
        ...
        public void OnMessageReceived()
        {
            ...
            switch (type)
            {
                case "Exec":
                {
                    ...
                    this._model.Get_listSaveJob()[saveJobNb].Execute(_viewModel,
                        this._model.Get_listSaveJob()[saveJobNb], this._model.Get_logFile(), this._model.Get_workFile(),
                        this._model, this);
                }
                    break;
                case "Dele":
                ...
                case "Edit":
                {
                    ...
                    {
                        this._model.Get_listSaveJob().Add(new M_SaveJob(i["Name"].ToString(),
                            i["SourceFilePath"].ToString(), i["TargetFilePath"].ToString(), i["Type"].Value<int>(),
                            i["State"].ToString(), 0, identationIndex));
                        identationIndex += 1;
                    }
        ...
        public void StartServer()
        {
            while (true)
            {
                ...
                Thread threadAccepterConnexion =
                    new Thread(() => this._socket = Server.AccepterConnexion(this._serverSocket));
        ...
        public void EcouterReseauEnContinue()
        {
            Thread threadEcouteReseau = new Thread(() =>
                this.Set_receivedMessage(this.EcouterReseau(this._socket, this._serverSocket)));
